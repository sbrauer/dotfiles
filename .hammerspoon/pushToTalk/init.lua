...
function onInputDeviceChanged(uid, name, scope, element)
  log.i("onInputDeviceChanged " .. name)
  if name ~= "vmvc" then
    return
  end
...
-- Sam 18-Oct-2018
-- Something keeps lowering my volume level, and I've had it!
-- Tweaking this to unmute to 100% all the time. Hear me now dammit!
  ...
  if mute then
    ...
    for index, device in ipairs(hs.audiodevice.allInputDevices()) do
      if inputVolumes[device:uid()] == nil then
        ...
        log.i('Unmuting audio: MAXIMUM LEVEL')
